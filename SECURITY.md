...
- A remote image download or page title request reveals your IP address to the host, exactly as opening the address would. With Web images set to Link with preview the plugin makes no request itself, but the pasted embed makes Obsidian load the picture from the web whenever the note is shown. Set Web images to Do nothing and turn off link title fetching if that matters for a particular vault.
