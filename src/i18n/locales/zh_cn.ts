...
export const STRINGS_ZH_CN: TranslationStrings = {
    ...
    notices: {
        ...
        imagesFailedNothingPasted: '{images}，因此没有粘贴任何内容。剪贴板中仍保留着它。',
    ...
    settings: {
        ...
        start: {
            ...
            supportName: '支持开发',
            supportDesc: '如果 Better Paste 对你有帮助，欢迎支持它的持续开发。',
        ...
        behavior: {
            ...
            autoCleanDesc: '每次粘贴都会应用规则。关闭后只能通过命令使用。单篇笔记可以用 "better-paste: false" 属性排除自己。',
            autoCleanAliases: ['自动', '启用', '禁用', '笔记', '排除', '属性', 'frontmatter'],
            showNoticesName: '粘贴内容被修改时显示提示',
            showNoticesDesc: '用一行说明清理了什么。无论此项如何设置，失败都会提示。',
        ...
        images: {
            ...
            savingDesc:
                '把粘贴的图片保存为本地文件，而不是留下外部图片链接。这包括 Safari 的“拷贝图像”、复制的网页内容中的图片，以及单独的图片地址。图片会保存到仓库的附件文件夹。选择“沿用来源名称”时：',
            savingAliases: ['下载', '附件', 'safari', '截图', '图片', '文件夹', '文件名', '宽度', '尺寸'],
            pageName: '图片处理',
            pageDesc: '文件名以及每篇笔记的图片宽度。',
            nameFormatName: '文件名',
            nameFormatDesc: '选择保存的图片文件如何命名。',
            ...
            sizePropertyDesc:
                '决定粘贴进笔记的图片宽度的 frontmatter 属性。使用该属性的笔记会由本插件接管截图粘贴，而不再交给 Obsidian。留空即为不启用。',
        ...
        links: {
            ...
            titlesDesc:
                '当剪贴板中只有一个非图片的网址时，会获取其页面标题并粘贴为 Markdown 链接。若已选中其他文本，则直接用作链接文字，不发起任何请求。若无法获取标题，则保留原始网址。',
            titlesAliases: ['标题', '页面', '网站', 'markdown 链接', '下载'],
            cleaningName: '清理粘贴的链接',
            cleaningDesc: '移除粘贴链接中的跟踪参数。带删除线的部分会被移除：',
            cleaningAliases: ['url', '跟踪', 'utm', '参数', '查询', '网站', '域名', 'youtube', '例外'],
            stripName: '移除哪些参数',
            stripDesc: '选择移除全部查询参数，还是只移除已知的跟踪参数。两种方式下站点规则都能保留参数。',
            stripAliases: ['utm', '跟踪', '查询', '参数'],
            stripAll: '除站点规则保留的以外，移除全部参数',
            stripTracking: '只移除已知的跟踪参数',
            rulesName: '保留参数的规则',
            rulesDesc: '在两种移除方式下都保留特定查询参数的站点规则。',
            rulesCount: { other: '{count} 个站点' },
            listName: '你的站点规则',
            listDesc:
                '{sites}已经处理好，并随插件一起保持更新。在这里添加你自己的站点规则，每行一条。“example.com”保留该站点的全部参数，“example.com: a, b”只保留这两个，“!example.com”则取消插件自带的一条规则。在“只移除已知的跟踪参数”方式下，其他参数本来就会保留，因此规则只会救回匹配的跟踪参数。子域名会自动匹配。',
            listShippedCount: { other: '{count} 个常见站点' },
            listAliases: ['域名', '例外', '白名单', 'youtube'],
            listInvalid: '不是站点名称：{values}',
            testerName: '试一试',
            testerDesc: '粘贴一个链接，看看这些规则会保留什么。',
            testerLabel: '要清理的链接',
        ...
        terminal: {
            heading: '终端文本',
            cleanupName: '清理终端输出',
            cleanupDesc: '把终端输出中被折断的行重新接起来，并去掉缩进。颜色代码会被移除。代码块、表格和列表项保持不变。',
            cleanupAliases: ['折行', '接行', 'ansi', '控制台', 'shell', '缩进', '项目符号', '列表', 'markdown'],
            pageName: '终端文本处理',
            pageDesc: '接行条件与项目符号字符。',
            rejoinName: '何时接回被折断的行',
            rejoinDesc: '把一行视为上一行延续所需的条件。',
            rejoinAliases: ['缩进', '折行', '激进', '保守', 'git log'],
            rejoinIndented: '仅当下一行有缩进时',
            rejoinAny: '只要上一行看起来是满的',
            rejoinNever: '从不接行，只去掉颜色代码和缩进',
            bulletsName: '项目符号字符',
            bulletsDesc: '决定终端输出中的项目符号（例如 •）是保留原样，还是转换成 Markdown 列表项。',
