...
export const STRINGS_JA: TranslationStrings = {
    ...
    notices: {
        ...
        imagesFailedNothingPasted: '{images}。そのため何も貼り付けていません。内容はクリップボードに残っています。',
    ...
    settings: {
        ...
        start: {
            ...
            supportName: '開発を支援する',
            supportDesc: 'Better Paste が役に立ったら、継続的な開発の支援をご検討ください。',
        ...
        behavior: {
            ...
            autoCleanDesc:
                '貼り付けのたびにルールを適用します。コマンドだけを使いたい場合はオフにしてください。個々のノートは "better-paste: false" プロパティで対象外にできます。',
            autoCleanAliases: ['自動', '有効', '無効', 'ノート', '除外', 'プロパティ', 'フロントマター', 'frontmatter'],
            showNoticesName: '貼り付けが変更されたときに通知を表示',
            showNoticesDesc: '何を整えたかを 1 行でまとめます。失敗はこの設定に関係なく必ず通知されます。',
        ...
        images: {
            ...
            savingDesc:
                '外部の画像リンクを残さず、貼り付けた画像をローカルファイルとして保存します。Safari の「画像をコピー」、コピーしたウェブ内容に含まれる画像、単独の画像アドレスが対象です。画像は保管庫の添付ファイルフォルダーに保存されます。「元の名前を使う」の場合:',
            savingAliases: [
                'ダウンロード',
                '添付ファイル',
                'safari',
            ...
            pageName: '画像の扱い',
            pageDesc: 'ファイル名とノートごとの画像幅。',
            nameFormatName: 'ファイル名',
            nameFormatDesc: '保存する画像ファイルの名前の付け方を選びます。',
            ...
            sizePropertyDesc:
                'ノートに貼り付ける画像の幅を決めるフロントマターのプロパティです。このプロパティを使うノートでは、スクリーンショットの貼り付けを Obsidian に代わってこのプラグインが処理します。空欄にすると無効になります。',
        ...
        links: {
            ...
            titlesDesc:
                'クリップボードに画像以外のウェブアドレスだけがある場合、ページタイトルを取得して Markdown リンクを貼り付けます。ほかに選択したテキストがあれば、通信せずにそれをラベルにします。タイトルを取得できないときは元のアドレスを残します。',
            titlesAliases: ['タイトル', 'ページ', 'ウェブサイト', 'markdown リンク', 'ダウンロード'],
            cleaningName: '貼り付けたリンクを整理',
            cleaningDesc: '貼り付けたリンクから追跡パラメーターを取り除きます。取り消し線の部分が削除されます:',
            cleaningAliases: ['url', '追跡', 'utm', 'パラメーター', 'クエリ', 'サイト', 'ドメイン', 'youtube', '例外'],
            stripName: '削除するパラメーター',
            stripDesc:
                'すべてのクエリパラメーターを削除するか、既知の追跡パラメーターだけを削除するかを選びます。サイトルールはどちらの方式でもパラメーターを残せます。',
            stripAliases: ['utm', '追跡', 'クエリ', 'パラメーター'],
            stripAll: 'サイトルールが残す場合を除き、すべてのパラメーター',
            stripTracking: '既知の追跡パラメーターのみ',
            rulesName: 'パラメーターを残すルール',
            rulesDesc: 'どちらの削除方式でも特定のクエリパラメーターを残すためのサイトルール。',
            rulesCount: { other: '{count} サイト' },
            listName: '自分のサイトルール',
            listDesc:
                '{sites}はすでに対応済みで、プラグインとともに最新に保たれます。独自のサイトルールを 1 行に 1 つずつ追加してください。「example.com」はそのサイトのすべてのパラメーターを残し、「example.com: a, b」はその 2 つだけを残し、「!example.com」はプラグイン同梱のルールを取り消します。「既知の追跡パラメーターのみ」の方式では、他のパラメーターは元から残るため、ルールは該当する追跡パラメーターだけを残します。サブドメインは自動的に一致します。',
            listShippedCount: { other: '{count} 件の主要サイト' },
            listAliases: ['ドメイン', '例外', 'ホワイトリスト', 'youtube'],
            listInvalid: 'サイト名ではありません: {values}',
            testerName: '試す',
            testerDesc: 'リンクを貼り付けると、これらのルールが何を残すか確認できます。',
            testerLabel: '整理するリンク',
        ...
        terminal: {
            ...
            cleanupDesc:
                'ターミナル出力で折り返された行をつなぎ直し、字下げを取り除きます。カラーコードは削除されます。コードブロック、表、リスト項目はそのままです。',
            cleanupAliases: ['折り返し', 'つなぐ', 'ansi', 'コンソール', 'シェル', '字下げ', '箇条書き', 'リスト', 'markdown'],
            pageName: 'ターミナルテキストの扱い',
            pageDesc: 'つなぎ直す条件と箇条書き記号。',
            rejoinName: '折れた行をつなぎ直す条件',
            rejoinDesc: 'ある行を前の行の続きとして扱うために必要な条件です。',
            rejoinAliases: ['字下げ', '折り返し', '積極的', '安全', 'git log'],
            rejoinIndented: '次の行が字下げされている場合のみ',
            rejoinAny: '上の行が埋まって見えるときは常に',
            rejoinNever: 'つなぎ直さず、コードと字下げだけを取り除く',
            bulletsName: '箇条書き記号',
            bulletsDesc: 'ターミナル出力の箇条書き記号 (• など) をそのまま残すか、Markdown のリスト項目に変換するかを決めます。',
        ...
        text: {
            ...
            commasName: 'カンマと引用符',
            commasDesc: '閉じる二重引用符の隣にカンマを置く位置を選びます。',
